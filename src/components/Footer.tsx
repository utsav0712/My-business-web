// src/components/Footer.tsx
"use client";
import React, { useState, useRef, useEffect } from "react";
import { motion } from "framer-motion";
import { MessageCircle, Facebook, Phone, Mail, X, MapPin } from "lucide-react";

const WA_NUMBERS = [
  { id: "wa1", label: "WhatsApp 1", number: "919837517668" },
  { id: "wa2", label: "WhatsApp 2", number: "919457991688" },
];
const FB_PAGE = "https://www.facebook.com/share/1BAp222ui6/";
const CALL_NUMBER = "+919837517668";
const MAIL_TO = "rajang851@gmail.com";

export default function Footer() {
  const [waOpen, setWaOpen] = useState(false);
  const [mapOpen, setMapOpen] = useState(false);
  const waRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    function onDoc(e: MouseEvent) {
      if (!waRef.current) return;
      if (!(e.target instanceof Node)) return;
      if (!waRef.current.contains(e.target)) setWaOpen(false);
    }
    document.addEventListener("click", onDoc);
    return () => document.removeEventListener("click", onDoc);
  }, []);

  const openWhatsApp = (num: string) => {
    const href = `https://wa.me/${num}`;
    window.open(href, "_blank", "noopener,noreferrer");
    setWaOpen(false);
  };

  const openFacebook = () =>
    window.open(FB_PAGE, "_blank", "noopener,noreferrer");
  const openCall = () => (window.location.href = `tel:${CALL_NUMBER}`);
  const openMail = () => (window.location.href = `mailto:${MAIL_TO}`);

  return (
    <footer className="relative bg-white text-gray-700 border-t border-gray-200">
      <div className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Brand + brief */}
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-indigo-600 to-sky-400 flex items-center justify-center text-white font-bold">
                GE
              </div>
              <div>
                <div className="font-semibold text-lg text-gray-900">
                  Utsav Crockery
                </div>
                <div className="text-xs text-gray-500">
                  Trusted quality crockery for homes and celebrations.
                </div>
              </div>
            </div>

            <p className="text-sm text-gray-600 max-w-sm">
              <strong className="text-gray-900">Trusted Retail Partner for 20+ Years</strong><br />
              At <strong className="text-gray-900">Utsav Crockery –</strong>, we bring elegance and utility together under one roof. For over two decades, our family-run store has been serving customers with a wide variety of crockery, kitchenware, and daily household essentials.
            </p>

            <div className="flex items-center gap-3 relative">
              {/* WhatsApp popover */}
              <div className="relative" ref={waRef}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setWaOpen((s) => !s);
                  }}
                  aria-expanded={waOpen}
                  aria-controls="wa-popover"
                  aria-label="Open WhatsApp options"
                  className="p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition inline-flex items-center gap-2"
                  title="WhatsApp"
                >
                  <MessageCircle className="w-5 h-5 text-green-600" />
                </button>

                {waOpen && (
                  <motion.div
                    id="wa-popover"
                    initial={{ opacity: 0, y: -6 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -6 }}
                    transition={{ duration: 0.12 }}
                    className="absolute left-0 md:right-0 mt-2 w-56 rounded-lg bg-white border border-gray-200 p-3 shadow-lg"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-xs text-gray-600">
                        Chat on WhatsApp
                      </div>
                      <button
                        onClick={() => setWaOpen(false)}
                        aria-label="Close WhatsApp menu"
                        className="p-1 rounded hover:bg-gray-100"
                      >
                        <X className="w-4 h-4 text-gray-500" />
                      </button>
                    </div>

                    <div className="flex flex-col gap-2">
                      {WA_NUMBERS.map((w) => (
                        <button
                          key={w.id}
                          onClick={() => openWhatsApp(w.number)}
                          aria-label={`Chat with ${w.label}`}
                          className="text-left px-3 py-2 rounded hover:bg-gray-100 transition flex items-center gap-3"
                        >
                          <MessageCircle className="w-4 h-4 text-green-600" />
                          <div>
                            <div className="text-sm text-gray-800">
                              {w.label}
                            </div>
                            <div className="text-xs text-gray-500">
                              Tap to open chat
                            </div>
                          </div>
                        </button>
                      ))}
                    </div>
                  </motion.div>
                )}
              </div>

              {/* Facebook */}
              <button
                onClick={openFacebook}
                aria-label="Open Utsav Crockery Facebook page"
                className="p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition"
                title="Facebook"
              >
                <Facebook className="w-5 h-5 text-blue-600" />
              </button>

              {/* Call */}
              <button
                onClick={openCall}
                aria-label="Call Utsav Crockery"
                className="p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition"
                title="Call"
              >
                <Phone className="w-5 h-5 text-indigo-600" />
              </button>

              {/* Mail */}
              <button
                onClick={openMail}
                aria-label="Send email to Utsav Crockery"
                className="p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition"
                title="Email"
              >
                <Mail className="w-5 h-5 text-rose-600" />
              </button>

              {/* Map */}
              <button
                onClick={() => setMapOpen(true)}
                aria-label="Open map location"
                className="p-2 rounded-md bg-gray-100 hover:bg-gray-200 transition"
                title="Find us on map"
              >
                <MapPin className="w-5 h-5 text-green-600" />
              </button>
            </div>
          </div>

          {/* Quick Links */}
          <div className="flex flex-col">
            <div className="font-semibold mb-4 text-gray-900">Quick Links</div>
            <nav className="flex flex-col gap-2 text-sm">
              {[
                "Home",
                "About",
                "Products",
                "Contact",
              ].map((link) => (
                <a
                  key={link}
                  href={`#${link.toLowerCase()}`}
                  className="text-gray-600 hover:text-gray-900"
                >
                  {link}
                </a>
              ))}
            </nav>
          </div>

          {/* Contact & newsletter */}
          <div className="flex flex-col">
            <div className="font-semibold mb-4 text-gray-900">Contact</div>
            <div className="text-sm text-gray-700">{MAIL_TO}</div>
            <div className="text-sm text-gray-700 mt-2">{CALL_NUMBER}</div>

            <div className="mt-6">
              <form
                className="flex gap-2"
                onSubmit={(e) => {
                  e.preventDefault();
                  alert("Thank you for subscribing!");
                }}
              >
                <input
                  aria-label="Email for newsletter"
                  type="email"
                  placeholder="Your email"
                  className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
                <button
                  type="submit"
                  className="rounded-lg bg-gradient-to-br from-indigo-600 to-sky-500 px-3 py-2 text-sm font-semibold text-white"
                >
                  Subscribe
                </button>
              </form>
            </div>
          </div>
        </div>

        <div className="mt-10 border-t border-gray-200 pt-6 flex flex-col md:flex-row items-center justify-between text-sm text-gray-500">
          <div>
            © 2025 Utsav Crockery. All rights
            reserved.
          </div>
          <div className="mt-3 md:mt-0">
            Made with ❤️ — Designed for businesses
          </div>
        </div>
      </div>

      {/* Map Modal */}
      {mapOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="relative w-[90%] max-w-3xl h-[400px] bg-white rounded-lg overflow-hidden shadow-lg">
            <button
              onClick={() => setMapOpen(false)}
              aria-label="Close map modal"
              className="absolute top-2 right-2 p-2 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-800 z-10"
            >
              <X className="w-5 h-5" />
            </button>

            <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3474.3456184382994!2d77.3121109752314!3d29.447916446587907!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390c2f758744e2e1%3A0x66321c644df45139!2sGanpati%20Enterprises!5e0!3m2!1sen!2sin!4v1756322785834!5m2!1sen!2sin"
              width="100%"
              height="100%"
              style={{ border: 0 }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
            />
          </div>
        </div>
      )}
    </footer>
  );
}
